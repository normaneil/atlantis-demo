  ...
  login(payload)
    .then((response) => {
      if (response.success) {
        ...
        store.setAuth(true);
        store.setUser(response.name, response.id, response.is_admin);
...
<template>
  <div class="flex items-center justify-center h-screen px-6 bg-gray-200">
    <div class="w-full max-w-sm bg-white rounded-md shadow-md">
      <div class="p-5">
        <form class="" @submit.prevent="handleSubmit">
          ...
          <div class="mt-6">
            <button
              type="submit"
              class="w-full px-4 py-2 text-sm text-center text-white bg-blueGray-800 rounded-md focus:outline-none"
              :disabled="loading"
            >
